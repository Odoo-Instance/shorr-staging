...
class ApprovalRequest(models.Model):
    ...
    has_stock_location = fields.Selection(related="category_id.has_stock_location")
    ...
    transfers_count = fields.Integer(string='Transfer Request Count', compute="_compute_transfer_request_count")

    def _compute_transfer_request_count(self):
        for rec in self:
            transfers = self.env['stock.picking'].sudo().search([('origin', '=', rec.reference_number)])
        ...
        if self.application == 'warehouse':
            ...
            if picking_type:
                ...
                transfer_request.message_post_with_view('mail.message_origin_link',
                                                        values={'self': transfer_request, 'origin': self},
                                                        subtype_id=self.env.ref('mail.mt_note').id)

    def action_view_transfer_request(self):
        document = self.env['stock.picking'].sudo().search([('origin', '=', self.reference_number)])
